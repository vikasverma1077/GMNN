...
class Trainer(object):
    ...
    def update_gs(self, inputs, target):
        ...
        #loss = self.criterion(logits, target[inputs])
        loss = -torch.mean(torch.sum(torch.log(logits) * target[inputs]), dim=-1)
        
        loss.backward()
        self.optimizer.step()
        return loss.item()

    def update_soft_aux(self, idx, target, mixup_layer):
        ...
        #loss = self.criterion(torch.softmax(logits, dim=-1), mixed_target)
        loss = -torch.mean(torch.sum(torch.log(torch.softmax(logits, dim=-1)) * mixed_target), dim=-1)
