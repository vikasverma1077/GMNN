...
def mixup_gnn_hidden(x, target, train_idx, alpha):
    ...
    """
    return x, target[train_idx], target[train_idx][permuted_train_idx], lam
...
class GSGCN(nn.Module):
    def __init__(self, opt, features, neighbors):
        ...
        opt_ = dict([('in', opt['num_feature']), ('out', opt['hidden_dim']), ('gcn', opt['gcn']), ('num_sample', 25), ('cuda', opt['cuda'])])
        ...
        opt_ = dict([('in', opt['hidden_dim']), ('out', opt['hidden_dim']), ('gcn', opt['gcn']), ('num_sample', 10), ('cuda', opt['cuda'])])
